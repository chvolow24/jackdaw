...
In case this matters to you: AI has for the most part **not** been used to make Jackdaw. I would estimate that about 200 lines of code, out of ~73k total (0.27%) were substantially generated by AI.
...
11. [Track effects](#track-effects)
	...
	8. [Reverb](#reverb)
	9. [Pitch shifter](#pitch-shifter)
	10. [Vibrato](#vibrato)
12. [Audio routes](#audio-routes)
...
16. [Special audio inputs](#special-audio-inputs)
...
The pitch shifter effect has three pitch adjustment sliders for different kinds of pitch control:

**`Broad (cents)`**
Spans a continuous two-octave range. Useful for large, continuous pitch slides.

**`Coarse (semitones)`**
Discrete semitone shifts in the same two-octave range. Useful for transposition to a new key.

**`Fine (cents)`**
A continuous whole-tone range. Useful for very small tuning adjustments.

The effect shift amount is the sum total of the shifts specified by each of these three sliders.

**`Phase fidelity vs freq fidelity`**
The last parameter determines (along with the shift amount) how closely spaced the individual slices of re-pitched audio are. Short, closely spaced slices -- a low value on this slider -- will minimize the overall latency introduced by the effect, and will keep transients from getting overly smeared/echoed in time. However, closely-spaced slices also introduce comb filtering, which sounds ugly and can obscure the actual pitches present in the original sound (poor "frequency fidelity"). Longer, more widely-spaced slices -- a high value on this slider -- preserve the notes better and tend to sound less ugly, but come at the cost of higher latency and generally messed-up timing (poor "phase fidelity"). Higher values are recommended for smoother, less time-sensitive sounds (e.g. vocals) and lower values are recommended for more time-sensitive sounds (drums or anything with timed, sharp transients). At low shift amounts, the tradeoffs are less severe in both directions. 

### Vibrato

A simple vibrato effect based on a modulated delay line. The `Depth` parameter displays the *upward* pitch offset in cents.

## Audio routes

By default, audio from each track is sent to the output device (specified in the "Output" panel):

```
............TRACK 1.............................
:                                              :
:  [ Clips ] --.-- [ Effects ] -- [ Vol/Pan ] -:--.- [ Out ]
:              |                               :  |
:  [ Synth ] --'                               :  |
:                                              :  |
................................................  |
...
By adding audio routes, you can send a track's audio output to another track, where it is treated as an input alongside that track's clips (if any) and synth (if active). For example:

```
[ Kick ] ----.-- [ Drum bus ] --.-- [ Master ] -- [ Out ]
             |                  |
[ Snare ] ---'                  |
             |                  |
[ Hi-hat ] --'                  |
...
`Drum bus` and `Master` here are regular Tracks, as are `Kick`, `Snare`, `Hi-hat`, `Piano`, and `Bass`. There is no separate "bus" concept in Jackdaw. You could add clips to those tracks too if you want to, which would not be possible on a typical bus.

You can use audio routes to accomplish parallel processing:

```
[ Drums ] --.-- [ Compressor 1 ] --.-- [ Master ] -- [ Out ]
            |                      |
            '-- [ Compressor 2 ] --'
            |                      |
            '-- [ Compressor 3 ] --'
...
To add an audio route, first select a track, then use one of the following:

<kbd>A-o</kbd> : **Open audio routes out page**<br>
<kbd>A-i</kbd> : **Open audio routes in page**<br>
<kbd>A-S-o</kbd> : **Quick add audio route out**<br>
<kbd>A-S-i</kbd> : **Quick add audio route in**<br>

The first two will open a tabview where you can view, add or remove a track's routes and adjust the gain of each route. On the 'routes out' page, you can also specify whether the track's output should be sent to the main out (audio device). Adding a new route out to a track will automatically uncheck that box.
