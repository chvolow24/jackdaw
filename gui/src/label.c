...
static void std_str_fn(char *dst, size_t dstsize, ValType t, void *target)
{
    ...
    Window *win)
{
    ...
    l->tb = textbox_create_from_str(l->str, lt, win->mono_font, 14, win);
    if (set_str_fn) {
...
{
    label->set_str_fn(label->str, label->max_len, label->val_type, label->target_obj);
