...
Layout *openfile_loop(Layout *lt) 
{
    ...
    if (!openfile) {
        ...
        openfile->label = create_text_from_str("Open file at:", 14, &(get_child_recursive(openfile_lt, "label")->rect), open_sans, white, BOTTOM_LEFT, true, main_win->rend);
        openfile->filepath = create_text_from_str(filepath_buffer, 254, &(get_child_recursive(openfile_lt, "filepath")->rect), open_sans, white, BOTTOM_LEFT, true, main_win->rend);
    }
