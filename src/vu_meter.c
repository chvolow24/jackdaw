...
VUMeter *vu_meter_create(Layout *container, bool horizontal, EnvelopeFollower *ef_L, EnvelopeFollower *ef_R)
{
	...
	red_part->h.value = 1.0f;
	red_part->w.type = SCALE;
	red_part->w.value = (amp_max - 1.0f) / amp_max;
    } else {
	red_part->w.type = SCALE;
    ...
    if (horizontal) {
	orange_part->x.type = STACK;
	orange_part->x.value = 0.0f;
    ...
    if (horizontal) {
	green_part->w.type = REVREL;
	green_part->w.value = 0;	
    } else {
